...
-(void)setTitle:(NSString *)title{
    if(title != nil && title.length>0){
        ...
        self.headerlabel.al_height = 60.0f ;
        self.tableView.tableHeaderView = self.headerlabel ;
    }else{
        self.headerlabel.al_height = 0.0f ;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.tintColor = [UIColor colorWithRed:20.0f/255.0 green:20.0f/255.0 blue:20.0f/255.0 alpha:1.0];
//        VCRColorMake(20, 20, 20, 1.0);
        self.separatorColor = [UIColor colorWithRed:200/255.0 green:200/255.0 blue:200/255.0 alpha:1.0];
//        VCRColorMake(200, 200, 200, 1.0);
    }
    return self;
}
...
-(void)addFuncItem:(NSString *)name
{
    ...
    [array addObject:item];
    self.datalist = array ;
    subView.al_height = [self getSubViewHeight] ;
    [subView reloadByData:self.datalist];
...
-(void)show{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (kDevice_Is_iPhoneX) {
            [self->subView setAl_y:RectH-self->subView.al_height-39];
        }else{
            [self->subView setAl_y:RectH-self->subView.al_height];
        }
